...
(defun test-mailbox-producers-consumers
  ...
  (let ((mbox    (make-mailbox))
	(counter (lem-mailbox::make-atomic-integer))
	(+sleep+  0.0001)
	    ...
	    (receivers
	      (flet ((process-msg (msg out)
			 ...
			 ((not (< -1 msg n-messages))
			  (funcall out nil))
			 (t
			  (lem-mailbox::atomic-incf counter)))))
		(append
	...
	(let ((garbage  0))
	  (flet ((wait-for (threads)
	    ...
	    `(:received . ,(lem-mailbox::atomic-get counter))))))))
...
;;     ;; the counter.
;;     (if (and received (<= (cdr received) 1000000))
;; 	(setf received '(:received . :ok))
;; 	received)
;;     (ok (equal '(:received . :ok) received))))
